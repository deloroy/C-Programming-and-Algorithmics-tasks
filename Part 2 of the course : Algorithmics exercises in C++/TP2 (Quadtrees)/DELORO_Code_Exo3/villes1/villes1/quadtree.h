...
//2 définitions permettant de faire tourner le header sur C++ <11
#define nullptr NULL
...
//Fonction calculant la profondeur d'un quadtree
template <typename T>
int profondeur(QuadTree<T>* &qtree){
    if (qtree==nullptr){
        return -1;
    }
    if (qtree->isLeaf()){
    ...
    if (qtree->isNode()){
        int prof_arbre=0;//est alors égale au maximum des profondeurs de ses fils
        for (int i=0;i<4;i++){
            int prof_sous_arbre=profondeur(qtree->son(i));
            if (prof_sous_arbre+1>prof_arbre){
                prof_arbre=prof_sous_arbre+1;
            }
        }
        return prof_arbre;
    }
