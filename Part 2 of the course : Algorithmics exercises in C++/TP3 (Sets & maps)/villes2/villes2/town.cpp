...
//Construit une ville fictive de nom "T" et de coordonnées géographiques (-1,-1)
Town::Town(){
    _name="T";
    _lat=-1;
    _lon=-1;
...
// Munit les villes d'un ordre total
// Etablit l'ordre lexicographique sur les villes selon (nom, x, y)
// Renvoit True si la ville A est avant la ville B
bool Town::operator<(const Town T) const{
    return (name()<T.name())||((name()==T.name())&&(Point2D(x(),y())<Point2D(T.x(),T.y())));
}
